...
class Forms {
	...
	public static function loadData( $form, $method,  &$errors = array() ) {
		$fields	= Settings::getConfig( $form );
	...
	/**
	 * Load form configuration and sets anti-XSRF field (each page refresh).
	 * Send to builder
	 ...
	 * @param array $inputs Form field configuration in JSON format
	 * @return array Input form fields and their attributes.
	 */
	public static function buildForm( $form, $data = null, $inputs = array() ) {
		if ( empty( $inputs ) ) {
			$inputs			= Settings::getConfig( $form );
		}
